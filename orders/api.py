...
def order_detail(request, pk):
    ...
    # صلاحية: العميل، المطعم، المندوب، الأدمن
    if not (
        order.client == user
...
def restaurant_confirm_order(request, pk):
    ...
    order = get_object_or_404(Order, pk=pk, restaurant__owner=request.user, order_type=Order.OrderType.FOOD)
